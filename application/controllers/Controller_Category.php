	...
	public function fetchCategoryData()
	{
		...
		foreach ($data as $key => $value) {
			...
			if(in_array('updateCategory', $this->permission)) {
				$buttons .= ' <a href="'.base_url('Controller_Category/update/'.$value['id']).'" class="btn btn-warning btn-sm"><i class="fa fa-pencil"></i></a>';
			}
	...
	public function update($id)
	{
        ...
        if ($this->form_validation->run() == TRUE) {
        	$data = array(
				...
				'research_and_development' => $this->input->post('research_and_development'),
				'covid_impact' => $this->input->post('covid_impact'),                    
        	);
                
        	$update = $this->model_category->update($data, $id);
        	if($update == true) {
        		$this->session->set_flashdata('success', 'Successfully updated');
        		redirect('Controller_Category/update/'.$id, 'refresh');
        	}
        	else {
