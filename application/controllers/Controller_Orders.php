...
class Controller_Orders extends Admin_Controller
{
	...
	*/
	public function fetchOrdersData()
	{
		...
		$cust_data = trim($this->input->post('customer_name'));

		//		$data = $this->model_orders->getOrdersData();
		$data = $this->model_orders->getOrdersByName($cust_data); //('Ramesh Ghule');
		//                $notpaid_amt = 0;
		$net_amount_tot = 0;
		$index = 0;
		...
		foreach ($data as $key => $value) {
			...
			if ($value['paid_status'] == 1) {
				...
				//                                $notpaid_amt += $value['net_amount'];
			}

			$index = $key;
			$net_amount_tot += $value['net_amount'];
	...
	public function create()
	{
		...
		if ($this->form_validation->run() == TRUE) {
			...
			if ($order_id) {
				...
				//                    $customer = $this->model_customers->getCustomersDataByMobile($customer_phone);
				$customer_id = $this->input->post('customer');
				$data = array(
					'store_id' => 1,
					'customer_id' => $customer_id,
					'amount' => (int) $this->input->post('net_amount_value'),
				...
				//                    if($this->input->post('paid_status') == 1){  //1 means status Paid
				if ($this->input->post('paid_amount') > 0) {  //it means some amt Paid
					$data = array(
						'store_id' => 1,
						'customer_id' => $customer_id,
						'amount' => (int) $this->input->post('paid_amount'),
						'Description' => 'PAID: order_id-' . $order_id,
						'order_id' => $order_id,
						'paid_status' => 'IN',
						'user_id' => $this->session->userdata('id')
					);
			...
			$this->data['customer'] = $this->model_customers->getCustomersData();
	...
	*/
	public function printDiv($id)
	{
		...
		if ($id) {
			...
			$Payments_data = $this->model_customers_payments->getPaymentsData_ByOrderId($id);

			$out_amount = 0;
			$in_amount = 0;
			if ($Payments_data) {
				foreach ($Payments_data as $k => $v) {
					if ($v['paid_status'] == 'OUT') {
						$out_amount += $v['amount'];
					} elseif ($v['paid_status'] == 'IN') {
						$in_amount += $v['amount'];
					}
				}
			} else {
				$out_amount = $order_data['net_amount'];
			}

			$balance_amount = $out_amount - $in_amount;
