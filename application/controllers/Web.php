...
class Web extends Cust_Controller {
	...
	public function contact() {
		if ($this->input->post()) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$Array['ip'] = $ip;
				$Array['website'] = "tryanglemarket.com";
				$Array['source'] = "Contact Us";
				// $url = "https://tryanglemarket.com/addLead";
				// $postData = $Array;
				// $ch = curl_init();
				...
				$message .= '<p>We have received your message and our representative will reach out to you at the earliest.<br/>Meanwhile, if you have any specific concerns/questions, please drop a line at <br/>sales@tryanglemarket.com <br/>Have a good day ahead!</p><br/>';
				$message .= '<p><b>Best Regards,</b><br/>';
				$message .= 'paypal  Team<br/>';
				$message .= 'US: +1 990999999999<br/>';
				$message .= 'E-mail: sales@tryanglemarket.com | Web: www.tryanglemarket.com</p>';

				$this->data_model->sendMail($to, $subject, $message);

				$to = 'sales@tryanglemarket.com';
	...
	public function categories() {
		...
		$data['category'] = $this->data_model->get(NULL, NULL, array('id', 'name', 'slug','img','description'), NULL, 'category');
		$data['page_name'] = 'Categories';
		$cond = array('status' => 1);
		$data['clients'] = $this->data_model->get($cond, NULL, array('id', 'name','img'), NULL, 'client');
		$this->render_template('cust/categories', $data);
	}
	...
	public function categoryDetails($slug = "", $page = 1) {
		...
		$data['page_title'] = 'Select Industry';
		// $this->data['reports_data'] = $this->model_reports->getReportsByCategory($category_id);
		$this->render_template('cust/report-listing', $data);
	}
	...
	public function buyNow($slug) {
		if ($_POST) {
			...
			$buynowfield['report'] = '<a href="https://www.tryanglemarket.com/report/'.$id.'">'.$_POST['reportTitle'].'</a>';
			$buynowfield['region'] = $res->continent;
			$buynowfield['ip'] = $ip;
			$buynowfield['website'] = "tryanglemarket.com";
			...
			$post = [
				...
				'report' => '<a href="https://www.tryanglemarket.com/report/'.$id.'">'.$_POST['reportTitle'].'</a>',
				'region' => $res->continent,
				'postal_code' => $res->postal_code,
				'subdivision' => $res->subdivision,
				'city' => $res->city,
			...
			if ($_POST['optionsRadios'] == "wireTransfer") {
				...
				if ($_POST['optionsRadios'] == 'ccAvenue') {
					...
					$data['itemInfo'] = [
						...
						"image" => "https://www.tryanglemarket.com/web_assets/images/logo.png",
						"prefill" => [
							"name" => $_POST['name'],
							"email" => $_POST['email'],
   ...
   //                  //Check success response
	...
	public function callKeywords($offset,$limit){
		$url = "https://www.tryanglemarket.com/unique-keywords/".$offset."/".$limit;
		// $ch = curl_init();
		// curl_setopt($ch, CURLOPT_URL, $url);
		// curl_setopt($ch, CURLOPT_RETURNTRANSFER, TRUE);
		// curl_setopt($ch, CURLOPT_HEADER, false);
